...
vi.mock('../../utils/telegramUtils', () => ({
  ...
  sendMessage_withInlineKeyboard: vi.fn(),
    updateMessage_withInlineKeyboard: vi.fn(),
}));
...
describe('transactionsHandler', () => {
  ...
  it('should return a message with all transactions and update the message when called with a page number', async () => {
    prisma.transaction.findMany.mockResolvedValue([
      {
        groupTransactionId: 1,
        type: 'REPAYMENT',
    ...
    await transactionsHandler(chatId, 1, "1", 10);
    expect(updateMessage_withInlineKeyboard).toHaveBeenCalledWith(
        chatId,
        "1",
        '<b>Transactions:</b>\nId: 1 Type: REPAYMENT \nFrom: user1 To: user2 \nAmount: $10 Description: Lunch\n\nId: 2 Type: EXPENSE \nFrom: user3 To: user4 \nAmount: $20 Description: Dinner\n\n',
        [[{text: "◀️", callback_data: `prev_1`}, {text: "▶️", callback_data: `next_1`}]]
    );
  });
});